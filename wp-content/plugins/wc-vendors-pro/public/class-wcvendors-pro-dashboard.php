...
class WCVendors_Pro_Dashboard {
	...
	public function get_dashboard_pages()  { 
		...
		$this->dashboard_pages[ 'settings' ] = array( 
			'slug'			=> 'settings', 
			'label'			=> __('Kitchen Settings', 'wcvendors-pro' ), 
			'actions'		=> array( )
		);
	...
	*/
	public function create_nav( ) { 
		...
		if ( !$viewstore_disabled ) { 
			$store_url 		= apply_filters( 'wcv_dashboard_view_store_url', array( 'label' => __( 'Preview Kitchen', 'wcvendors-pro' ),'slug' => WCVendors_Pro_Vendor_Controller::get_vendor_store_url( get_current_user_id() ) ) ); 
