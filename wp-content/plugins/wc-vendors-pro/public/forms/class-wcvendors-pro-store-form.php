	...
	*/
	...
	public static function store_form_tabs(){ 
		...
		$store_tabs = apply_filters( 'wcv_store_tabs', array(
			'store' 			=> array(
				'label'  => __( 'Kitchen Details', 'wcvendors-pro'), 
				'target' => 'store',
				'class'  => array(),
			), 
			'branding' 			=> array( 
				'label'  => __( 'Kitchen Photos', 'wcvendors-pro'), 
				'target' => 'branding',
				'class'  => array(),
			), 
			'payment' 		=> array( 
...
function store_bank_account( ){ 
	if ( class_exists( 'WCVendors_Pro' ) ){ 
		$key = '_wcv_custom_settings_bank_account'; 
		$value = get_user_meta( get_current_user_id(), $key, true ); 
		// Bank Name
		WCVendors_Pro_Form_Helper::input( array(  
			'id' 				=> $key, 
			'label' 			=> __( 'Bank Account Number', 'wcvendors-pro' ), 
			'placeholder' 			=> __( 'Numeric account number', 'wcvendors-pro' ), 
			'desc_tip' 			=> 'true', 
...
function store_bank_ifsc( ){ 
	if ( class_exists( 'WCVendors_Pro' ) ){ 
		$key = '_wcv_custom_settings_bank_ifsc'; 
		$value = get_user_meta( get_current_user_id(), $key, true ); 
		// Bank Name
		WCVendors_Pro_Form_Helper::input( array(  
			'id' 				=> $key, 
			'label' 			=> __( 'Bank IFSC Code', 'wcvendors-pro' ), 
			'placeholder' 			=> __( 'IFSC Code', 'wcvendors-pro' ), 
			'desc_tip' 			=> 'true', 
			'description' 			=> __( 'Your bank branch IFSC Code', 'wcvendors-pro' ), 
			'type' 				=> 'text', 
			'value'				=> $value, 
			)
		);
	...
	public static function vacation_mode( ) { 
		...
		if ( ! self::$settings_store[ 'vacation_mode' ] ) { 
			...
			WCVendors_Pro_Form_Helper::input( apply_filters( 'wcv_vacation_mode', array( 
						...
						'wrapper_start' 	=> '<div class="wcv-cols-group wcv-horizontal-gutters" style="color:red"><div class="all-100">', 
						'wrapper_end' 		=> '</div>', 
						'value'				=> $vacation_mode
						) 
			) );
